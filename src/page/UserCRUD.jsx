...
/**
 * UserCRUD component is responsible for managing user data, including displaying a table of users,
 * editing user details, and deleting users. It uses various hooks and components from the `notification`
 * and `axios` libraries to handle notifications and API requests.
 *
 * @component
 * @example
 * return (
...
const UserCRUD = () => {
  ...
  const openNotification = (user_data) => {
    ...
    if (user_data.deleted === "true") {
      return api.open({
        ...
        description: `You have successfully suspended user with user id ${user_data._id}
      ...
      return api.open({
        ...
        description: `You have successfully reverted user with user id ${user_data._id}
        username: ${user_data.username}`,
        duration: 4,
      });
    }
  ...
  const deleteUser = async (record) => {
    try {
      setLoading(true);
      const delete_user = await axios.put(`${API_URL}/admin/deleteuser`, {
        record,
      });
